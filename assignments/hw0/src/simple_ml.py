...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    return np.mean(np.log(np.exp(Z).sum(axis=1)) - Z[range(len(Z)), y])
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    for i in range(0, num_samples, batch):
        if i + batch > num_samples:
            ...
            x_batch = X[i : i + batch]
            y_batch = y[i : i + batch]
        Z = np.exp(x_batch @ theta) / np.exp(x_batch @ theta).sum(axis=1)[:, np.newaxis]
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(0, num_samples, batch):
        ...
        normalize = lambda x: x / x.sum(axis=1)[:, np.newaxis]
        Z1 = relu(x_batch @ W1)
